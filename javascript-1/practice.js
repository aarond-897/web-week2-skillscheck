...
function bigOrSmall(arr) {
	let answers = arr.map(val => val > 100 ? 'big' : 'small')
	return answers;
}
...
myNumbers.forEach((val, i) => myNumbersIndex.push(i))
...
let enemies = people.filter(val => val.friend === false)
...
let totallyAwesome = people.reduce(((accm, curr) => accm + curr.awesomeLevel), 0)